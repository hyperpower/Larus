...
int vector_add() {
    ...
    d_a  = clCreateBuffer(context,  CL_MEM_READ_ONLY,  sizeof(vt) * count, NULL, &err);
    checkError(err, "Creating buffer d_a");

    d_b  = clCreateBuffer(context,  CL_MEM_READ_ONLY,  sizeof(vt) * count, NULL, &err);
    checkError(err, "Creating buffer d_b");

    d_c  = clCreateBuffer(context,  CL_MEM_WRITE_ONLY, sizeof(vt) * count, NULL, &err);
    checkError(err, "Creating buffer d_c");

    // Write a and b vectors into compute device memory
    err = clEnqueueWriteBuffer(commands, d_a, CL_TRUE, 0, sizeof(vt) * count, h_a, 0, NULL, NULL);
    checkError(err, "Copying h_a to device at d_a");

    err = clEnqueueWriteBuffer(commands, d_b, CL_TRUE, 0, sizeof(vt) * count, h_b, 0, NULL, NULL);
    ...
    err = clEnqueueReadBuffer( commands, d_c, CL_TRUE, 0, sizeof(vt) * count, h_c, 0, NULL, NULL );
    if (err != CL_SUCCESS)
    {
...
int vector_dot() {
    ...
    // Create the compute kernel from the program
    ko_vadd = clCreateKernel(program, "dot_local_reduce_kernel", &err);
    checkError(err, "Creating kernel");
    ...
    err |= clSetKernelArg(ko_vadd, 3, sizeof(uint), &count);
    checkError(err, "Setting kernel arguments");
    ...
    global = count;
    err = clEnqueueNDRangeKernel(commands, ko_vadd, 1, NULL, &global, &workgroupsize, 0, NULL, NULL);
    //err = clEnqueueNDRangeKernel(commands, ko_vadd, 1, NULL, &global, NULL, 0, NULL, NULL);
    checkError(err, "Enqueueing kernel");
    ...
    // Test the results
    correct = 0;
    vt*  h_res = (vt*) calloc(count, sizeof(vt));
    vt sum=0;
    for(i = 0; i < count; i++)
    {
    	h_res[i] = h_a[i] * h_b[i];     // assign element i of a+b to tmp
        sum += h_res[i];          // compute deviation of expected and output result
    }
    vt sumd = 0;
    for(i = 0; i < count; i++){
    	sumd += h_c[i];
    	//cout<<"i "<<i <<"  "<<h_c[i]<<" "<<h_res[i]<<endl;
    }
    cout<< "dot  "<<sumd<<"  "<<sum<<endl;
