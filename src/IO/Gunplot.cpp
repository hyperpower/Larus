...
bool Gnuplot::set_GNUPlotPath(const std::string &path) {
	...
	std::string tmp = path + "/" + this->m_sGNUPlotFileName;

#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__TOS_WIN__)
	if ( this->file_exists(tmp,0) ) // check existence
#elif defined(unix) || defined(__unix) || defined(__unix__) || defined(__APPLE__)
	if (this->file_exists(tmp, 1)) // check existence and execution permission
#endif
			{
		this->m_sGNUPlotPath = path;
		return true;
	} else {
		this->m_sGNUPlotPath.clear();
		return false;
	}
}
...
Gnuplot& Gnuplot::set_label(const std::string& label) {
	std::ostringstream cmdstr;

	cmdstr << "set label " << label;
	cmd(cmdstr.str());
...
//
Gnuplot& Gnuplot::plot_equation(const std::string &equation,  //equation
...
//
Gnuplot& Gnuplot::plotfile_x(const std::string &filename,   //name
		const unsigned int column,     //column
		const std::string &title)      //title
		{
	//
...
bool Gnuplot::file_available(const std::string &filename) {
	...
	if (this->file_exists(filename, 0)) // check existence
			{
		if (!(this->file_exists(filename, 4))) { // check read permission
...
std::string Gnuplot::create_tmpfile(std::ofstream &tmp) {
	...
	//
	if (this->tmpfile_num == GP_MAX_TMP_FILES - 1) {
		std::ostringstream except;
...
void Gnuplot::remove_tmpfiles() {
	if ((tmpfile_list).size() > 0) {
		...
		this->tmpfile_num -= tmpfile_list.size();
	}
}
