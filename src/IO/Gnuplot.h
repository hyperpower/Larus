...
//declare classes in Larus namespace
namespace Larus {
...
public:
	...
	Gnuplot(const std::vector<double> &x,    //vector
			const std::string &title = "",   //title
			const std::string &style = "points",    //style
			const std::string &labelx = "x",   //label
			const std::string &labely = "y");   //label
	...
	Gnuplot& set_label(const std::string& label);
	/// set equal ratio
	...
	Gnuplot& plot_xy(   //  type has [] and size()
			const X& x, //
			const Y& y, //
			const std::string &title = "");  //
	...
	Gnuplot& plotfile_xy_err(const std::string &filename,
			const unsigned int column_x = 1, const unsigned int column_y = 2,
			const unsigned int column_dy = 3, const std::string &title = "");
	///   from data
	...
	Gnuplot& plot_image(const unsigned char *ucPicBuf,
			const unsigned int iWidth, const unsigned int iHeight,
			const std::string &title = "");
...
Gnuplot& Gnuplot::plot_x(const X& x, const std::string &title) {
	if (x.size() == 0) {
		throw GnuplotException("std::vector length equal to 0");
		return *this;
	}
...
//------------------------------------------------------------------------------
//
//  Plots a 2d graph from a list of doubles: x y with gap
//
template<typename X, typename Y>
Gnuplot& Gnuplot::plot_2_jump(  //
		const X& x,    //
		const Y& y,    //
		const unsigned int& gap, //
		const std::string &title) {
...
Gnuplot& Gnuplot::plot_7( //
		...
		const ARR& val,  //
		const std::string &str) {  //
	if (xc.size() <= 0 || yc.size() <= 0 || xm.size() <= 0 || xp.size() <= 0
			|| ym.size() <= 0 || yp.size() <= 0 || val.size() <= 0) {
		throw GnuplotException("Array too small");
		return *this;
	}
	unsigned int n = xc.size();
	if (n != yc.size() || n != xm.size() || n != xp.size() || n != ym.size()
			|| n != yp.size() || n != val.size()) {
		throw GnuplotException("Length of the Array differs");
		return *this;
	}
	// inline data
	std::ostringstream sst;
	//
	sst << "plot \"-\" using 1:2:3:4:5:6:7 " << str;
	cmd(sst.str());
	sst.str("");
	typename ARR::const_iterator iter[7];
	iter[0] = xc.begin();
	iter[1] = yc.begin();
	iter[2] = xm.begin();
	iter[3] = xp.begin();
