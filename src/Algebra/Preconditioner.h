...
void ICFactor(const arrayListV<LarusDef::size_type> &pntr,
		const arrayListV<LarusDef::size_type> &indx, arrayListV<VALUE> &val)
{
...
public:
	...
	MatrixSCR<VALUE> getPre()
	{
		MatrixSCR<VALUE> res(dim_[0], dim_[1], nz_, val_, pntr_, indx_);
		return res;
	}
	MatrixSCR<VALUE> getPreTrans()
	{
		MatrixSCR<VALUE> res(dim_[0], dim_[1], nz_, val_, pntr_, indx_);
		res.trans();
		return res;
...
public:
	...
	DiaPre(const MatrixSCR<VALUE> &M, int inv) :
			diag_(M.iLen())
	{
		inv_ = inv;
		int flagi = -1;
...
public:
	ILUPre(const MatrixSCR<VALUE> &A) :
	...
	MatrixSCR<VALUE> getPreL()
	{
		MatrixSCR<VALUE> res(dim_[0], dim_[1], l_nz_, l_val_, l_rowptr_, l_colind_);
		return res;
	}
	MatrixSCR<VALUE> getPreU()
	{
		MatrixSCR<VALUE> res(dim_[0], dim_[1], u_nz_, u_val_, u_rowptr_, u_colind_);
		return res;
	}
};
