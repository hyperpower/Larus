...
/* A NumberlikeArray<TYPE> object holds a heap-allocated array of TYPE with a
...
public:
	...
	NumberlikeArray(Index c) :
			cap(c), len(0)
	{
		blk = (cap > 0) ? (new TYPE[cap]) : NULL_PTR;
	}
	...
	// Copy constructor
	NumberlikeArray(const NumberlikeArray<TYPE> &x);

	// Assignment operator
	void operator=(const NumberlikeArray<TYPE> &x);

	// Constructor that copies from a given array of blocks
	NumberlikeArray(const TYPE *b, Index blen);
	 ...
	 * override. */
	bool operator ==(const NumberlikeArray<TYPE> &x) const;
...
template<class TYPE>
const unsigned int NumberlikeArray<TYPE>::N = 8 * sizeof(TYPE);

template<class TYPE>
void NumberlikeArray<TYPE>::allocate(Index c)
{
...
template<class TYPE>
void NumberlikeArray<TYPE>::allocateAndCopy(Index c)
{
	// If the requested capacity is more than the current capacity...
	if (c > cap) {
		TYPE *oldBlk = blk;
...
template<class TYPE>
NumberlikeArray<TYPE>::NumberlikeArray(const NumberlikeArray<TYPE> &x) :
		len(x.len)
{
	// Create array
	cap = len;
...
template<class TYPE>
void NumberlikeArray<TYPE>::operator=(const NumberlikeArray<TYPE> &x)
{
...
template<class TYPE>
NumberlikeArray<TYPE>::NumberlikeArray(const TYPE *b, Index blen) :
		cap(blen), len(blen)
{
	// Create array
	blk = new TYPE[cap];
...
template<class TYPE>
bool NumberlikeArray<TYPE>::operator ==(const NumberlikeArray<TYPE> &x) const
{
	if (len != x.len)
		// Definitely unequal.
		return false;
...
protected:
	// Helpers
	template<class TYPE> void initFromPrimitive(TYPE x);
	template<class TYPE> void initFromSignedPrimitive(TYPE x);
public:
...
protected:
	// Helpers
	template<class TYPE> TYPE convertToSignedPrimitive() const;
	template<class TYPE> TYPE convertToPrimitive() const;
public:
	...
	// See BigInteger.cc.
	template<class TYPE>
	friend TYPE convertBigUnsignedToPrimitiveAccess(const BigUnsigned &a);
};
...
template<class TYPE>
void BigUnsigned::initFromPrimitive(TYPE x)
{
	if (x == 0) {
		return; // NumberlikeArray already initialized us to zero.
	} else {
...
template<class TYPE>
void BigUnsigned::initFromSignedPrimitive(TYPE x)
{
	if (x < 0)
...
template<class TYPE>
TYPE BigUnsigned::convertToPrimitive() const
{
	if (len == 0)
...
template<class TYPE>
TYPE BigUnsigned::convertToSignedPrimitive() const
{
	TYPE x = convertToPrimitive<TYPE>();
	if (x >= 0)
		return x;
	else
		throw "BigUnsigned::to(Primitive): "
